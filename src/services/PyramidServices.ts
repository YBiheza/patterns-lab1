...
export class PyramidService {
    static calculateArea(pyramid: Pyramid): number {
        const xValues = pyramid.vertices.map(v => v.x);
        const zValues = pyramid.vertices.map(v => v.z);
      
        const foundWidth : number = Math.max(...xValues) - Math.min(...xValues);
        const foundLength : number = Math.max(...zValues) - Math.min(...zValues);
    ...
    static calculateVolume (pyramid: Pyramid) {
        const xValues = pyramid.vertices.map(v => v.x);
    ...
    static calculateVolumeParts (pyramid: Pyramid) {
    ...
    static FoundationOnThePlane (pyramid: Pyramid): boolean {
    ...
    static isValidPyramidShape(pyramid: Pyramid): boolean {
        const isVerticalHeight =
        pyramid.apex.x === pyramid.center.x &&
        ...
        const middleX =
        (Math.min(...pyramid.vertices.map((v) => v.x)) +
        Math.max(...pyramid.vertices.map((v) => v.x))) / 2;
        const middleZ =
        (Math.min(...pyramid.vertices.map((v) => v.z)) +
        ...
        const distances = pyramid.vertices.map(
            (v) =>
                Math.sqrt(
                    Math.pow(pyramid.apex.x - v.x, 2) +
        ...
        const isSymmetric =
            Math.abs(distances[0] - distances[1]) < 0.0001 &&
